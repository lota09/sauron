...
가상환경·의존성·secrets·디스코드 채널·DB 시딩까지 한 번에 세팅한다. 여러 번 돌려도 안전.
...
단계:
  ...
  4) setup_guild --dry → 만들 게 있으면 확인 후 실제 생성(통합·감시·학과 채널/역할)
  5) DB 초기화: init/seed_db.py (schema + depts_seed.csv의 60개 학과 upsert)
...
def step_venv():
    ...
    if os.path.exists(py):
        ...
        if r.returncode != 0 or not os.path.exists(py):
            print("    ✗ venv 생성 실패 (python3-venv 설치 필요할 수 있음: apt install python3.12-venv)")
            sys.exit(1)
        print(f"    → 생성됨: {name}")
    return py, True
...
def step_pip(py):
    ...
    if r.returncode != 0:
        print("    ✗ pip 설치 실패 — 로그 확인 후 재실행하세요.")
        sys.exit(1)
...
def step_setup_guild(py, can_run):
    print("\n[4] 디스코드 채널/역할 (setup_guild)")
    if not can_run:
        print("    → 토큰/서버ID 미비로 건너뜀. 나중에: python -m notify.setup_guild")
        return
    ...
    if r.returncode != 0 or "[오류]" in out or "길드 ID 없음" in out:
        print("    ⚠ dry 실행 실패(토큰/서버ID/봇 초대/네트워크 확인). 세팅은 건너뜁니다.")
        return
    ...
    if yn("    위 항목을 이대로 생성하며 진행하시겠습니까?", False):
        run([py, "-m", "notify.setup_guild"], cwd=ROOT)
    else:
        print("    → 건너뜀. 나중에: python -m notify.setup_guild")


def step_db(py):
    print("\n[5] DB 초기화 (schema + 학과 시드)")
...
def step_seed_notices(py):
    ...
    if r.returncode != 0:
        print("    ⚠ 시딩 중 오류(네트워크/사이트 확인). 재실행 가능(멱등).")
